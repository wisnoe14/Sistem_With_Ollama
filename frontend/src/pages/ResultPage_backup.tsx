...
const ResultPage: React.FC = () => {
  ...
  // Helper function untuk render winback-specific fields
  const renderWinbackFields = () => (
    <>
      <div className={`rounded-xl p-5 flex flex-col gap-1 border ${getStatusColor('keputusan', prediction.keputusan)}`}>
        ...
        <span className="text-lg font-bold">{prediction.keputusan || '-'}</span>
      </div>
      ...
      <div className="bg-indigo-50 rounded-xl p-5 flex flex-col gap-1 border border-indigo-100">
        ...
        <span className="text-lg font-bold text-indigo-900">{prediction.probability ? `${prediction.probability}%` : '-'}</span>
      </div>
      ...
      <div className="bg-orange-50 rounded-xl p-5 flex flex-col gap-1 border border-orange-100 md:col-span-2">
        <span className="text-xs text-gray-500 font-medium">Alasan</span>
        <span className="text-lg font-bold text-orange-900">{prediction.alasan || '-'}</span>
      </div>
  ...
  // Helper function untuk render telecollection fields
  const renderTelecollectionFields = () => (
    <>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 py-8 px-2">
        <div className="w-full max-w-xl bg-white p-8 rounded-3xl shadow-2xl border border-gray-100 space-y-8 animate-fade-in-down">
          <div className="flex flex-col items-center mb-6">
            ...
            <p className="text-gray-500 text-base">Berikut detail data pelanggan</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-blue-50 rounded-xl p-5 flex flex-col gap-1 border border-blue-100">
              ...
              <span className="text-lg font-bold text-blue-900">{customer_name}</span>
            </div>
            <div className="bg-blue-50 rounded-xl p-5 flex flex-col gap-1 border border-blue-100">
            ...
            <div className="bg-red-50 rounded-xl p-5 flex flex-col gap-1 border border-red-100 md:col-span-2">
              ...
              <span className="text-lg font-bold text-red-900">{status_dihubungi}</span>
            </div>
          </div>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-50 to-purple-50 py-8 px-2">
      <div className="w-full max-w-xl bg-white p-8 rounded-3xl shadow-2xl border border-gray-100 space-y-8 animate-fade-in-down">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {topic === 'winback' ? renderWinbackFields() : renderTelecollectionFields()}
        </div>
        <div className="flex flex-col gap-4 mt-4">
          <button
            onClick={() => navigate('/Home')}
